  ...
  try {
    const response = await fetch("/api/msn/session", {
      body: JSON.stringify({
        ...
        nick,
        password,
        personalMessage: isGusDev ? "@gus.dev" : "",
      }),
          ...
          await channel.send({
            ...
            payload: {
              contact: {
                ...
                message: profile.personalMessage || (profile.isAdmin ? "@gus.dev" : ""),
                nick: profile.nick,
                status: "online",
              },
  ...
  const contacts = useMemo(() => {
    ...
    const onlineContacts: MsnContact[] = freshOnlineProfiles
      ...
      .map((onlineProfile) => ({
        ...
        message: onlineProfile.personalMessage || (onlineProfile.isAdmin ? "@gus.dev" : ""),
